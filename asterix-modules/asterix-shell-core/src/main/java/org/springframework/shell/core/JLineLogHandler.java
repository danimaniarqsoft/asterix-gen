	...
	private String toDisplay(final LogRecord event) {
		...
		if (ansiSupported) {
			...
			if (event.getLevel().intValue() >= Level.SEVERE.intValue()) {
				...
				ansi.a(Attribute.NEGATIVE_ON).a(threadName).a(Attribute.NEGATIVE_OFF).fg(Color.MAGENTA).a(eventString)
						.reset();
			}
			else if (event.getLevel().intValue() >= Level.INFO.intValue()) {
				ansi.a(Attribute.NEGATIVE_ON).a(threadName).a(Attribute.NEGATIVE_OFF).fg(Color.GREEN).a(eventString).reset();
			}
			else {
