...
public abstract class AbstractShell extends AbstractShellStatusPublisher implements Shell {
	...
	public File getHome() {
		...
		Assert.isTrue(!f.exists() || (f.exists() && f.isDirectory()), "Path '" + f.getAbsolutePath() + "' debe ser un directorio o no debe de existir");
		if (!f.exists()) {
			f.mkdirs();
		}
		Assert.isTrue(f.exists() && f.isDirectory(), "Path '" + f.getAbsolutePath() + "' no es un directorio; por favor especifíque roo.home adecuadamente");
		return f;
	}
